...
`,componentJoin=(e,t)=>e.reduce((o,n,r)=>(r&&o.push({...t,key:r}),o.push(n),o),[]),KeycodeMap=getKeycodes().flatMap(e=>e.keycodes).reduce((e,t)=>({...e,[t.code]:t}),{}),optimizeKeycodeSequence=e=>pipeline(e,convertCharacterTaps,trimLastWait,mergeConsecutiveWaits,foldKeydownKeyupKeys,convertToCharacterStreams),cleanKeycodeSequence=e=>pipeline(e,mergeConsecutiveWaits),MacroRecorder=({selectedMacro:e,setUnsavedMacro:t,saveMacro:o,undoMacro:n,isDelaySupported:r})=>{const[s,a]=reactExports.useState(!0),[i,c]=reactExports.useState(!1),[l,d]=reactExports.useState(!1),[u,C]=reactExports.useState(!!document.fullscreenElement),{smartOptimizeEnabled:_,recordDelaysEnabled:h}=useAppSelector(getMacroEditorSettings),p=useDispatch(),[K,y]=useKeycodeRecorder(i,h),g=reactExports.useRef(null),m=reactExports.useCallback(async f=>{c(f),f?(await navigator.keyboard.lock(),y([]),a(!1),d(!0)):(navigator.keyboard.unlock(),_&&y(optimizeKeycodeSequence(K)),d(!1))},[K,c]),S=reactExports.useCallback(()=>{o(""),a(!0),d(!1)},[y,o]),b=reactExports.useCallback(()=>{n(),y([]),a(!0),d(!1)},[n]);reactExports.useEffect(()=>{a(!0),d(!1),y([])},[e]);const T=()=>[...s?e??[]:K],M=reactExports.useMemo(()=>{let f,L=T();return s||!l||!_?f=L:f=optimizeKeycodeSequence(L),f},[K,s,_,l,e]);reactExports.useEffect(()=>{M&&t(sequenceToExpression(M))},[M]);const A=reactExports.useCallback(()=>{s&&a(!1)},[s]),v=reactExports.useCallback(f=>{const L=T();L.splice(f,1),y(cleanKeycodeSequence(L)),A()},[M,e,K,s]),I=reactExports.useCallback((f,L)=>{const w=T();w.splice(f,1,[RawKeycodeSequenceAction.Delay,L]),y(cleanKeycodeSequence(w)),A()},[M,e,K,s]),B=reactExports.useMemo(()=>componentJoin(M.map(([f,L],w)=>{const R=getSequenceItemComponent(f);return jsx(Deletable,{index:w,deleteItem:v,disabled:i,children:RawKeycodeSequenceAction.Delay!==f?jsx(R,{children:f===RawKeycodeSequenceAction.CharacterStream?componentJoin(String(L).split(" ").map((O,F)=>jsx("span",{children:O},F)),jsx("span",{style:{fontFamily:"fantasy, cursive, monospace"},children:"␣"})):Array.isArray(L)?L.map(O=>getSequenceLabel(KeycodeMap[O])??O).join(" + "):getSequenceLabel(KeycodeMap[L])}):jsx(WaitInput,{index:w,value:Number(L),updateValue:I})},`${w}-${f}`)}),jsx(SequenceLabelSeparator,{})),[M]);reactExports.useEffect(()=>{const f=()=>{C(!!document.fullscreenElement)};return document.documentElement.addEventListener("fullscreenchange",f),()=>{m(!1),document.documentElement.removeEventListener("fullscreenchange",f)}},[C]);const P=reactExports.useCallback(()=>{document.fullscreenElement?document.exitFullscreen&&(m(!1),document.exitFullscreen()):document.documentElement.requestFullscreen()},[m]);return jsxs(Fragment,{children:[jsx(MacroSequenceContainer,{ref:g,$isModified:!s,children:B.length?B:jsx(NoMacroRecorded,{children:"No macro recorded yet..."})}),jsx("div",{style:{border:"none",maxWidth:960,width:"100%",display:"flex",justifyContent:"center",transform:"translate(-0px, -21px)"},children:jsx(MacroEditControls,{isFullscreen:u,isEmpty:!e||!e.length,optimizeRecording:_,recordDelays:h,isRecording:i,addText:()=>{},deleteMacro:S,toggleOptimizeRecording:()=>{p(setMacroEditorSettings({smartOptimizeEnabled:!_}))},toggleRecordDelays:()=>{p(setMacroEditorSettings({recordDelaysEnabled:!h}))},toggleFullscreen:P,undoChanges:b,saveChanges:()=>o(),hasUnsavedChanges:!s,recordingToggleChange:m,isDelaySupported:r})})]})},TextArea=styled.textarea`
  box-sizing: border-box;
  background: var(--bg_control);
  padding: 5px 10px;
