...
export function isCustomKeycodeByte(byte, basicKeyToByte) {
  return byte >= basicKeyToByte._QK_KB && byte <= basicKeyToByte._QK_KB_MAX;
}
export function getCustomKeycodeIndex(byte, basicKeyToByte) {
  return byte - basicKeyToByte._QK_KB;
}
export function isMacroKeycodeByte(byte, basicKeyToByte) {
  return byte >= basicKeyToByte._QK_MACRO && byte <= basicKeyToByte._QK_MACRO_MAX;
}
export function getMacroKeycodeIndex(byte, basicKeyToByte) {
...
export function getOtherMenu(basicKeyToByte) {
  const keycodes = Object.keys(basicKeyToByte).filter((key) => !key.startsWith("_QK")).filter((key) => !keycodesList.map(({code}) => code).includes(key)).map((code) => ({
