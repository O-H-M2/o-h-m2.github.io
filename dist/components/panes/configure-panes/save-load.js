...
import deprecatedKeycodes from "../../../utils/key-to-byte/deprecated-keycodes.js";
...
export const Pane = () => {
  ...
  const loadLayout = (file) => {
    ...
    reader.onload = async () => {
      ...
      const keymap = saveFile.layers.map((layer) => layer.map((key) => {
        var _a;
        return getByteForCode(`${(_a = deprecatedKeycodes[key]) != null ? _a : key}`, basicKeyToByte);
      }));
      await dispatch(saveRawKeymapToDevice(keymap, selectedDevice));
      if (saveFile.encoders) {
        await Promise.all(saveFile.encoders.map((encoder, id) => Promise.all(encoder.map((layer, layerId) => {
          var _a, _b;
          return Promise.all([
            selectedDevice.api.setEncoderValue(layerId, id, false, getByteForCode(`${(_a = deprecatedKeycodes[layer[0]]) != null ? _a : layer[0]}`, basicKeyToByte)),
            selectedDevice.api.setEncoderValue(layerId, id, true, getByteForCode(`${(_b = deprecatedKeycodes[layer[1]]) != null ? _b : layer[1]}`, basicKeyToByte))
          ]);
        }))));
      }
