...
export const Test = () => {
  ...
  const useMatrixTest = async (protocol) => {
    if (startTest && api && selectedDefinition) {
      ...
      try {
        ...
        for (let offset = 0; offset < rows; offset += rowsPerQuery) {
          const querySize = Math.min(rows * bytesPerRow - newFlat.length, bytesPerRow * rowsPerQuery);
          newFlat.push(...await api.getKeyboardValue(KeyboardValue.SWITCH_MATRIX_STATE, protocol >= 12 ? [offset] : [], querySize));
        }
        const keysChanges = newFlat.some((val, byteIdx) => val ^ (flat[byteIdx] || 0));
        if (!keysChanges) {
  ...
  return /* @__PURE__ */ React.createElement(TestPane, null, /* @__PURE__ */ React.createElement(Grid1Col, null, /* @__PURE__ */ React.createElement(FlexCell, {
    ...
    onChange: (val) => {
      ...
      if (val) {
        setSelectedKeys({});
        useMatrixTest(selectedDevice.protocol);
      } else {
        setSelectedKeys({});
      }
    }
